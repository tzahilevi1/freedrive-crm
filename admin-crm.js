...
(function () {
  ...
  //  wa.me מפנה ל-web.whatsapp.com, והדפדפן מאפס את שם הלשונית בניווט
  //  חוצה-מקורות — לכן target בלבד לא מספיק והלחיצה השנייה פתחה עוד
  //  לשונית. שומרים הפניה לחלון: ניווט של חלון זר מותר (קריאה ממנו לא),
  //  ולכן אפשר להטיס אותו לכתובת החדשה במקום לפתוח חדש.
  var waWin = null;
  window.C2B_openWa = function (url) {
    if (!url) return false;
    //  רק דומיין הווטסאפ הרשמי. ההפניה לחלון נשמרת כדי לאפשר שימוש
    //  חוזר, ולכן היעד חייב להיות מוגבל ולא כתובת שרירותית.
    if (!/^https:\/\/(wa\.me|api\.whatsapp\.com|web\.whatsapp\.com)\//.test(url)) {
      window.open(url, '_blank', 'noopener'); return false;
    }
    try {
      if (waWin && !waWin.closed) { waWin.location.href = url; waWin.focus(); return false; }
    } catch (e) { /* החלון נסגר או נחסם — נופלים לפתיחה חדשה */ }
    waWin = window.open(url, 'fd-whatsapp');
    return false;
  };
  ...
  var LEAD_COLS = [
    ...
    { key: 'whatsapp', label: 'וואטסאפ', w: 84, sortable: false, cell: function (l) { var wa = waLink(l.phone); return '<td>' + (wa ? '<a class="wa-ic" href="' + wa + '" target="fd-whatsapp" rel="noopener" title="פתח וואטסאפ (משתמש בלשונית קיימת)" onclick="event.stopPropagation();return window.C2B_openWa(this.href)">💬</a>' : '—') + '</td>'; } },
  ...
  function renderLeadCard(lead, acts, tasks, docs, deals, pays, urls, audits) {
    ...
    var actBtns = getActionCfg().filter(function (c) { var a = metaByK[c.k]; return c.on !== false && a && a.roles.indexOf(role) >= 0; }).map(function (c) {
      ...
      if (a.k === 'wa') return wa ? '<a class="btn btn-ghost btn-sm" href="' + wa + '" target="fd-whatsapp" rel="noopener" title="נפתח בלשונית הווטסאפ הקיימת" onclick="return window.C2B_openWa(this.href)">' + a.icon + ' ' + lbl + '</a>' : '';
      ...
      db.from('quick_messages').select('*').eq('active', true).order('sort', { ascending: true }).then(function (r) {
        ...
        $('qmList').querySelectorAll('.qm-send').forEach(function (b) {
          b.addEventListener('click', function () {
            ...
            var opened = false;
            if (waNum && waText) { window.C2B_openWa('https://wa.me/' + waNum + '?text=' + encodeURIComponent(waText)); opened = true; }
  ...
  //  ---- עמלת מנהלת תיקי לקוחות (שני מודלים) ----
  //  מודל 1: חתימה באותו יום שהעסקה נוצרה → 200₪, אחרת → 100₪.
  ...
  var ACCT_COLS = [
    ...
    //  שאר שדות העסקה — זמינים לבחירה בבורר העמודות (ברירת מחדל: מוסתרים)
    ...
    C.txtCol('spec', 'מפרט / הערות עסקה', { w: 240 }),
  ...
  window.C2B_renderAccounting = function () {
    ...
    loading();
    Promise.all([
      db.from('deals').select('id,created_at,lead_id,order_no,form_type,status,salesperson,client_name,client_phone,client_email,client_address,client_id,invoice_name,car_make,car_model,car_year,car_trim,car_engine,car_gearbox,car_color,car_price,down_total,down_initial,down_balance,monthly,delivery_days,balance_to_pay,addons,vat_included,discount_pct,discount_amt,total,paid,spec,notes,stage,checklist,financing,tradein,charge_amount,acct_status,acct_notes,commission,brand,signed_at,cancel_reason,contract_type,updated_at,has_contract,has_signature,purchase_price').in('stage', ['signed', 'collection', 'ordered', 'delivered', 'cancelled']).is('deleted_at', null).order('created_at', { ascending: false }).limit(3000),   // הנהלת חשבונות — כל שדות העסקה (למעט כבדים/פנימיים)
  ...
  function acctWorkspace(deals, pays, prof, lname, docs, urls) {
    ...
    var cancelRows = cancelDeals.sort(function (a, b) { return String(b.updated_at || b.created_at || '').localeCompare(String(a.updated_at || a.created_at || '')); }).map(function (d) {
      ...
      var finSt = (d.financing && d.financing.status) ? esc(d.financing.status) : '<span class="muted">—</span>';
      return '<tr data-lead="' + (d.lead_id || '') + '" style="cursor:pointer"><td><b>#' + esc(d.order_no || '—') + '</b></td><td>' + esc(d.client_name || '—') + '</td><td>' + esc(((d.car_make || '') + ' ' + (d.car_model || '')).trim() || '—') + '</td><td>' + nis(d._tot || d.total) + '</td><td style="color:var(--ok);font-weight:700">' + nis(paid) + '</td><td>' + (paid > 0 ? '<span style="color:var(--danger);font-weight:800">להחזיר ' + nis(paid) + '</span>' : '<span class="muted">אין החזר</span>') + '</td><td>' + finSt + '</td><td class="muted">' + esc(d.cancel_reason || '—') + '</td></tr>';
    }).join('');
  ...
  function contractHTML(d, sig, ownership, tplBody) {
    ...
    //  בהסכם מוצג מחיר הרכב כפי שהוא בפרטי הרכב המוזמן, בלי חישוב.
    //  קודם הוצג מחיר הרכב ועוד יתרת המקדמה, מה שיצר סכום
    //  שלא מופיע בשום מקום אחר במערכת ולא ניתן לאימות מול כרטיס העסקה.
    var carPrice = Number(d.car_price) || 0;
    ...
    return '<div style="font-family:Arial,sans-serif;line-height:1.75;width:100%;color:#111;font-size:13px;direction:rtl;text-align:right;overflow-wrap:break-word;word-break:break-word">' +
      ...
      // הישות המשפטית מגיעה מהתצורה (.env). היא הייתה מוטמעת בקוד עם החברה של
      // התבנית המקורית — כלומר כל הסכם נחתם מול ח.פ של חברה אחרת.
      (LEGAL_ENTITY ? '<p style="text-align:center;color:#444;margin:0 0 6px;font-size:13px">' +
        esc(LEGAL_ENTITY).replace(/ /g, '&nbsp;') + '</p>' : '') +
      ...
      '<div style="display:grid;grid-template-columns:1fr 1fr;column-gap:28px;direction:rtl;margin:0 0 14px">' +
        ...
        fld('תשלום ראשוני *', d.down_total ? nis(d.down_total) : '') + fld('מחיר הרכב', carPrice ? nis(carPrice) : '') +
      '</div>' +
  ...
  function contractViewInner(lead, deal, justSaved, tplList, chosenTpl) {
    ...
    if (deal.id) {
      ...
      var signBase = 'https://crm.freedrive.co.il/sign.html';
      var signUrl = null;
      function withUrl(cb) {
  ...
  //  note = משפט שמסביר מאיפה המספר הגיע, כדי שלא יהיה פער שבין
  //  מה שכתוב בכרטיס לבין מה שרואים ברשימה.
  //  _date מוצג לכל שורה וגם קובע את המיון (החדש למעלה),
  //  אלא אם השורה נשאה _sort מפורש. שורה בלי id אינה לחיצה.
  function leadsPopup(title, list, note) {
    var rows = list.slice().sort(function (a, b) {
      if (a._sort != null || b._sort != null) return (a._sort || 0) - (b._sort || 0);
      return new Date(b._date || 0) - new Date(a._date || 0);
    });
    C.openDrawer('<div class="dw-head"><h3 style="margin:0">' + esc(title) + ' <span class="muted" style="font-size:13px">(' + rows.length + ')</span></h3></div>' +
      '<div class="dw-body">' + (note ? '<div class="dw-note">' + esc(note) + '</div>' : '') +
      (rows.length ? rows.map(function (l) {
        var meta = l._meta || [l.phone, l.car, l.brand, l._extra];
        meta = meta.filter(Boolean).map(function (x) { return esc(x); }).join(' · ');
        return '<div' + (l.id ? ' data-lead="' + l.id + '"' : '') + ' style="padding:11px 12px;border-bottom:1px solid var(--line);border-radius:8px' + (l.id ? ';cursor:pointer' : '') + '"' +
          (l.id ? ' onmouseover="this.style.background=\'var(--surface-2)\'" onmouseout="this.style.background=\'\'"' : '') + '>' +
          ...
          (l._date ? '<div class="dw-row-date">' + esc((l._dateLabel || 'נוצר') + ' ' + fmt(l._date)) + '</div>' : '') +
          '</div>';
  ...
  window.C2B_renderDashboard = function (opts) {
    ...
    Promise.all([
      leadsQ, db.from('tasks').select('id,title,due_at,done,lead_id,created_at'), db.from('appointments').select('status'),
  ...
  function drawDashboard() {
    ...
    //  המדד מכסה 100% מהלידים. ליד שטרם נענה נספר לפי הזמן שהוא כבר
    //  ממתין — אחרת ליד שאיש לא נגע בו פשוט נעלם מהמדד, והממוצע היה
    //  משתפר דווקא כשמזניחים לידים.
    var rtWaiting = leads.filter(function (l) { return !l.first_response_at; });
    var rts = leads.map(function (l) { return C.respMins(l.created_at, l.first_response_at || new Date()); });
    ...
    view(
      ...
      '<div class="cards" style="margin-top:14px">' +
        ...
        C.stat('זמן תגובה', avgRt ? C.respTxt(avgRt) : '—', null, 'rt',
          'כל הלידים · ' + rtWaiting.length + ' טרם נענו · בלי שעות סגירה') + C.stat('משימות פתוחות', openTasks, null, 'tasks', 'הכל מה שטרם בוצע — כולל אלה שבאיחור') +
    ...
    //  כל כרטיס נגזר מרשימה מסוימת, והלחיצה מחזירה בדיוק את
    //  אותה רשימה — אחרת המספר והפירוט יסתרו זה את זה.
    var kLeadById = {}; allLeads.forEach(function (l) { kLeadById[l.id] = l; });
    function kLead(l) { return { id: l.id, name: l.name, phone: l.phone, car: l.car, brand: l.brand, status: l.status, _date: l.created_at }; }
    function kDeal(d, dateField, label) {
      var l = kLeadById[d.lead_id] || {};
      return { id: d.lead_id, name: d.client_name || l.name, phone: l.phone,
        car: ((d.car_make || '') + ' ' + (d.car_model || '')).trim() || l.car,
        brand: l.brand, status: l.status, _extra: d.total ? nis(d.total) : '',
        _date: dateField === 'signed' ? dealAt(d) : d.created_at, _dateLabel: label };
    }
    var rangeTxt = fltLabel(dashRange);
    var KPI = {
      today: function () { return ['לידים חדשים היום',
        allLeads.filter(function (l) { return new Date(l.created_at || 0).getTime() >= todayS; }).map(kLead),
        'לפי מועד יצירת הליד · כרטיס של היום ולכן אינו מושפע מהמסנן העליון']; },
      signedToday: function () { return ['נחתמו היום',
        signed.filter(function (d) { return new Date(dealAt(d) || 0).getTime() >= todayS; }).map(function (d) { return kDeal(d, 'signed', 'נחתם'); }),
        'לפי מועד החתימה · כרטיס של היום ולכן אינו מושפע מהמסנן העליון']; },
      leads: function () { return ['סה"כ לידים', leads.map(kLead), rangeTxt]; },
      deals: function () { return ['עסקאות חתומות',
        deals.map(function (d) { return kDeal(d, 'signed', 'נחתם'); }),
        'חתומות ולא מבוטלות · ' + rangeTxt]; },
      meetings: function () { return ['פגישות נקבעו',
        leads.filter(function (l) { return (l.status || 'new') === 'meeting_set'; }).map(kLead),
        'לידים בסטטוס “נקבעה פגישה” · ' + rangeTxt]; },
      quotes: function () { return ['הצעות פתוחות',
        allDeals.filter(function (d) { return !d.has_signature && d.status !== 'cancelled' && d.stage !== 'cancelled'; })
          .map(function (d) { return kDeal(d, 'created', 'נפתח'); }),
        'תיקים שנפתחו וטרם נחתמו · כל הזמנים, ללא תלות במסנן']; },
      conv: function () { return ['אחוז סגירה — ' + conv + '%',
        deals.map(function (d) { return kDeal(d, 'signed', 'נחתם'); }),
      ...
      rt: function () {
        var rows = leads.map(function (l) {
          ...
          var end = l.first_response_at || new Date();
          var mins = C.respMins(l.created_at, end);
          var wall = C.respMins(l.created_at, end, true);
          var r = kLead(l);
          r._extra = (l.first_response_at ? 'נענה תוך ' : 'ממתין כבר ') + C.respTxt(mins) +
            (wall > mins + 1 ? ' (שעון קיר ' + C.respTxt(wall) + ' — כלל שעות סגירה)' : '') +
            (l.first_response_at ? '' : ' ⚠ טרם נענה');
          //  הממתינים קודם, ואחריהם האיטיים — שניהם דורשים פעולה
          r._sort = -mins - (l.first_response_at ? 0 : 1e9); return r;
        });
        return ['זמן תגובה ראשון', rows,
          'ממוצע ' + C.respTxt(avgRt) + ' על כל ' + rows.length + ' הלידים · ' + rtWaiting.length +
          ' טרם נענו ונספרים לפי זמן ההמתנה · נמדד בשעות פעילות · ' + rangeTxt];
      },
      ...
      tasks: function () { return ['משימות פתוחות',
        tasks.filter(function (t) { return !t.done; }).map(function (t) {
          var l = kLeadById[t.lead_id] || {};
          var over = t.due_at && new Date(t.due_at).getTime() < Date.now();
          return { id: t.lead_id || null, name: t.title || 'משימה',
            _meta: [l.name ? 'לקוח: ' + l.name : null,
                    t.due_at ? (over ? '⚠ באיחור — ' : 'למועד ') + fmt(t.due_at) : 'ללא מועד'],
            _date: t.created_at };
        }),
        'כל המשימות שטרם סומנו כבוצעו · ללא תלות במסנן']; }
    };
    C.$('view').querySelectorAll('[data-kpi]').forEach(function (el) {
      el.addEventListener('click', function () {
        var f = KPI[el.dataset.kpi]; if (!f) return;
  ...
  function drawBlock(k) {
    ...
    //  הרשימות הנפתחות מציגות תאריך יצירה לכל שורה
    function leadRowD(l) { return { id: l.id, name: l.name, phone: l.phone, car: l.car, brand: l.brand, status: l.status, _date: l.created_at }; }
    ...
    if (k === 'chart') {
      ...
      C.$('dashStatus').querySelectorAll('[data-status]').forEach(function (tr) { tr.addEventListener('click', function () { var kk = tr.dataset.status; leadsPopup(stDef(kk).label, leads.filter(function (l) { return (l.status || 'new') === kk; }).map(leadRowD), fltLabel(blockR['status'])); }); });
    } else if (k === 'stage') {
      ...
      C.$('dashBrand').querySelectorAll('[data-brand]').forEach(function (tr) { tr.addEventListener('click', function () { var b = tr.dataset.brand; leadsPopup('מותג: ' + b, leads.filter(function (l) { return (l.brand || 'לא ידוע') === b; }).map(leadRowD), fltLabel(blockR['brand'])); }); });
    } else if (k === 'agent') {
      ...
      C.$('dashAgent').querySelectorAll('[data-agent]').forEach(function (tr) { tr.addEventListener('click', function () { var n = tr.dataset.agent; leadsPopup('סוכן: ' + n, leads.filter(function (l) { return (prof[l.assigned_to] || 'לא שויך') === n; }).map(leadRowD), fltLabel(blockR['agent'])); }); });
    } else if (k === 'source') {
